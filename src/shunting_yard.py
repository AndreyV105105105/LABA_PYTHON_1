#! env python3
from src.tokenizator import get_tok, is_number
...
def translate_indx_to_rpn(expr):
    ...
    while expr:
        ...
        if old_tok == ")" and tok == "(":
            raise ValueError(
                f'Пропущен оператор между скобкой и скобкой: "{old_tok}", "{tok}"'
            )

        # Пропущен оператор между числом и открывающей скобкой
        if is_number(old_tok) and tok == "(":
            raise ValueError(
                f'Отсутствие бинарного знака между числом и скобкой: "{old_tok}", "{tok}"'
            )

        # Пропущен оператор между закрывающей скобкой и числом
        if is_number(tok) and old_tok == ")":
            raise ValueError(
                f'Отсутствие бинарного знака между скобкой и числом: "{old_tok}", "{tok}"'
            )
        ...
        if is_op(tok):
            ...
            while (
                op_stack
                and is_op(op_stack[-1])
                and (prio[op_stack[-1]] >= prio[tok])
                and associativity[tok] == "left"
            ) or (
                op_stack
                and is_op(op_stack[-1])
                and (prio[op_stack[-1]] > prio[tok])
                and associativity[tok] == "right"
            ):
